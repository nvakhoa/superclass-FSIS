...
def create_superclass_model(cfg, args) -> dict:
    """
    """
    # This is a training function so don't call it during eval mode
    assert not args.eval_only
    ...
    # If we've loaded the correct model then it should have the 'base' amount of classes in the box_predictor
    # assert base_model.roi_heads.box_predictor.cls_score.weight.data.shape[0] == base_class_number, f'{base_model.roi_heads.box_predictor.cls_score.weight.data.shape}'
    ...
    IDs = (ID_MAP, BASE_ID_MAP, NOVEL_ID_MAP)
    top_gamma = base_cfg.MODEL.ROI_HEADS.TOP_K
    superclass_map = _create_superclass(few_shot_dataset, IDs, top_gamma, base_model, base_cfg)
    
    merge_superclass_map = []
    base_merge = []
    for key, val in superclass_map.items():
        if len(val) == 1:
            base_merge.extend(val)
        else:
            merge_superclass_map.append(val)
    merge_superclass_map.append(base_merge)


    superclass_cfg = copy.deepcopy(cfg)
    superclass_cfg.defrost()
    superclass_cfg.MODEL.ROI_HEADS.NUM_CLASSES = final_class_number
    superclass_cfg.MODEL.ROI_HEADS.SUPER_CLASS = merge_superclass_map
    superclass_cfg.MODEL.ROI_HEADS.NAME = "StandardSuperClassROIHead"
    superclass_cfg.MODEL.ROI_HEADS.OUTPUT_LAYER = "SMS_LR"

    
    # WRITE SUPER-CLASS CONFIG 
    superclass_model, superclass_cfg = ddu.build_model_from_cfg(superclass_cfg)
    superclass_cfg.MODEL.WEIGHTS=os.path.join(cfg.OUTPUT_DIR, 'model_init.pth')
    write_config(args.config_file, superclass_cfg, args.src1, args.num_gpus)



    # SAVE SUPER-CLASS INIT WEIGHTS
    new_model = {'model': superclass_model.state_dict(), 'iteration': 0}


    _write_initialization_model(new_model, cfg)
    return new_model

    
def write_config(base_config_path, superclass_cfg, saved_cfg_path, num_gpus):
    import yaml
    ...
    result_config['MODEL']['ROI_HEADS']['SUPER_CLASS'] = superclass_cfg.MODEL.ROI_HEADS.SUPER_CLASS
    result_config['MODEL']['ROI_HEADS']['OUTPUT_LAYER'] = superclass_cfg.MODEL.ROI_HEADS.OUTPUT_LAYER
    result_config['MODEL']['ROI_HEADS']['NUM_CLASSES'] = superclass_cfg.MODEL.ROI_HEADS.NUM_CLASSES
    result_config['MODEL']['ROI_HEADS']['NAME'] = superclass_cfg.MODEL.ROI_HEADS.NAME

    result_config['MODEL']['WEIGHTS'] = superclass_cfg.MODEL.WEIGHTS

    print(f'Writing to {saved_cfg_path}')

    print("Writing config!")
    os.makedirs(os.path.split(saved_cfg_path)[0], exist_ok=True)
    with open(saved_cfg_path, 'w') as fp:
...
def _create_superclass(few_shot_dataset, IDs, top_gamma, feature_extractor, cfg):
    ...
    (ID_MAP, BASE_ID_MAP, NOVEL_ID_MAP) = IDs
    ID_MAP_INVERT = {val:key for key, val in ID_MAP.items()}
    novel_class_id = list(NOVEL_ID_MAP.values())
    ...
    with torch.no_grad():  # Potentially not needed if we set model to eval mode
        for idx in range(class_number):  # Loop over all the classes
            class_id = ID_MAP_INVERT[idx]
            # if class_id not in novel_class_id: continue

            examples_per_shot = 1
            u_c = []
            for _ in range(examples_per_shot):
                feature_list = [feature_extractor.feature_output_extract_gts_one_image(simple_mapper(shot)) for shot in class_to_img[idx]]

                u_c.extend(feature_list)


            u_c = torch.cat(u_c, dim=0)[None,...]

            # ICLR

            if class_id in novel_class_id:
                novel_features.append(u_c)


    novel_features = torch.cat(novel_features, dim=0)
    # remove information of background
    novel_features = novel_features[...,:-1].mean(1, keepdim=True)

    
    indx = voting(novel_features, top_gamma).cpu().detach().numpy()
    ...
    # print(indx.shape)
    # assert 2==1
    # indx = torch.argsort(novel_features, dim=2).cpu().detach().numpy
    SUPER_CLASS = {i: [ID_MAP[BASE_ID_MAP[i]]] for i in range(60)}
    SUPER_CLASS[60] = [80]
    for i, id_clses in enumerate(indx):
        novel_id_cls = ID_MAP[NOVEL_ID_MAP[i]]
        for ins in id_clses:
            for id_cls in ins:
                SUPER_CLASS[id_cls].append(novel_id_cls)


    roi_feat_dir = os.path.join(cfg.OUTPUT_DIR, 'roi_feat')
    os.makedirs(roi_feat_dir, exist_ok=True)
    torch.save(roi_features, os.path.join(roi_feat_dir,'feat.pth'))

    return SUPER_CLASS

def voting(novel_features, top_gamma=1):
    indx = torch.argsort(novel_features, dim=-1, descending=True)
    indx = indx.cpu().detach().numpy().tolist()
    dim = novel_features.shape

    scorce = torch.zeros(dim[0], dim[-1])
    for i, values in enumerate(indx):
        for ins in values:
            for scr, id_suppper_cls in enumerate(ins[::-1]):
                scorce[i][id_suppper_cls] += scr+1
            
    indx = torch.argsort(torch.tensor(scorce), descending=True, dim=-1)
    indx = indx[..., None, :top_gamma]

    return indx


def _write_initialization_model(trained_model: dict, cfg):
    logger.info(f'Writing init_checkpoint file to {cfg.OUTPUT_DIR}')


    with open(os.path.join(cfg.OUTPUT_DIR, 'model_init.pth'), 'wb') as model_file:
        logger.info(f'Writing model_init.pth')
        torch.save(trained_model, model_file)
        if 'all' in cfg.DATASETS.TEST[0]:
            logger.info(f'Using *all* dataset, writing model_reset_combine.pth as well.\n'
                        f'This is identical to model_init.pth, just renamed.')
