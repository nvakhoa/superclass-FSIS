...
class GeneralizedRCNN(nn.Module):
    ...
    def feature_output_extract_gts_one_image(self, image_dict) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Compute the feature extraction for the ground_truth proposals in one image.
        Essentially, for one image the following steps are computed:
        - Feature extraction through model backbone.
        ...
        feats = [features[f] for f in self.roi_heads.in_features]
        box_feat = self.roi_heads.box_pooler(feats, [x.gt_boxes for x in gt_instances])
        box_features = self.roi_heads.box_head(box_feat)

        pred_class_logits, pred_proposal_deltas = self.roi_heads.box_predictor(box_features)
